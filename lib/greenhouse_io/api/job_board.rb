module Greenhouse
  class JobBoard
    ...
    def initialize(api_token = nil, default_options = {})
      ...
      @organization = default_options.delete(:organization) || Greenhouse.configuration.organization
    end
    ...
    def post_to_job_board_api(url, options)
      ...
      if response.code == 200
        response.include?("success") ? parse_json(response) : raise(Greenhouse::Error.new(response["reason"]))
      else
        raise Greenhouse::Error.new(response.code)
      end
    end
